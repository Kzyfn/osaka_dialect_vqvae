...
def train(epoch, model, train_loader, loss_function, optimizer, f0=False):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        loss = loss_function(
            recon_batch, tmp[1], z_mu, z_unquantized_logvar#recon_batch, tmp[1][:, lf0_start_idx], z_mu, z_unquantized_logvar
        )
        ...
        if f0:
            with torch.no_grad():
                f0_loss += rmse(
                    ...
                    tmp[1].detach().cpu().numpy().reshape(-1),#tmp[1][:, lf0_start_idx].detach().cpu().numpy().reshape(-1)
                )
        optimizer.step()
        del tmp
    ...
    with torch.no_grad():
        for i, data, in enumerate(test_loader):
            ...
            f0_loss += rmse(
                ...
                tmp[1].cpu().numpy().reshape(-1),#tmp[1][:, lf0_start_idx].cpu().numpy().reshape(-1)
            )
            del tmp
